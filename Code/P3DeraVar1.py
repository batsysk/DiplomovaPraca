...
from torchvision.models import resnet, resnet50, resnet34, resnet101, resnet152, resnext50_32x4d, resnext101_32x8d, \
    resnext101_64x4d, wide_resnet101_2
import torch
...
# Definition of model architecture based on ResNet
class ResNet199(nn.Module):
    def __init__(self, pretrained_model_path, num_classes=25):
        super(ResNet199, self).__init__()

        # Loading ResNet50 as the base model
        base_model = wide_resnet101_2(weights=None)

        # Modifying the fully connected layer to match the size expected by the checkpoint
        in_features = base_model.fc.in_features
        base_model.fc = nn.Linear(in_features, 2048)
        ...
        # Removing of weight and bias from the state dictionary to match architecture
        ...
        # Replacing of the last fully connected layer
...
pretrained_model_path = './model_P3D-ResNet-199_Kinetics/p3d-16-16-640-best.pth.tar'
# pretrained_model_path = './model_P3D-ResNet-199_Kinetics-600/p3d199-12-16-k2-best.pth.tar'
num_classes = 25
model = ResNet199(pretrained_model_path, num_classes)
...
# Defining transformation into tensor format
transform = transforms.Compose([
    transforms.ToTensor(),
])

# Opening of CSV file and writing tested data
with open('predictionsP3DKineticsVar1.csv', 'w', newline='') as csvfile:
    ...
    for video_name in video_names:
        ...
        while video_capture.isOpened():
            ...
            frame_tensor = frame_tensor.to(device) if torch.cuda.is_available() else frame_tensor

            with torch.no_grad():
                outputs = model(frame_tensor)

            predicted_category_index = torch.argmax(outputs).item()
            all_predictions.append(predicted_category_index)

        # Releasing of the video capture
        video_capture.release()

        # Temporal pooling: Calculating of the most common prediction across all frames
        if all_predictions:
            predicted_category_index = max(set(all_predictions), key=all_predictions.count)

            # Printing the results
            predicted_category = category_dict.get(predicted_category_index, "Unknown")
